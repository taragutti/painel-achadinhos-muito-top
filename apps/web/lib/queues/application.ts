...
export async function queueWorkspaceView() {
  const data = await queueWorkspace();
  return {
    paused: data.paused,
    ...
    queues: data.queues.map((queue) => ({
      ...
      name: queue.name,
      status: queue.status,
      startsAt: queue.startsAt?.toISOString() ?? null,
      dailyStartTime: queue.dailyStartTime,
      dailyEndTime: queue.dailyEndTime,
      itemsPerBatch: queue.itemsPerBatch,
      intervalMinutes: queue.intervalMinutes,
      secondsBetweenItems: queue.secondsBetweenItems,
      repeatEnabled: queue.repeatEnabled,
      repeatCooldownHours: queue.repeatCooldownHours,
      nextRunAt: queue.nextRunAt?.toISOString() ?? null,
      channels: queue.targets.map((target) => ({
      ...
      items: queue.items.map((item) => ({
        ...
        productId: item.publication.productId,
...
export async function runQueueAction(
  ...
  if (input.action === "REMOVE_ITEM")
    return repo.removeItem(input.itemId);
