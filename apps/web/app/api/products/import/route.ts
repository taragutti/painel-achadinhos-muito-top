import { MockProductImporter, ProductImportService } from "@achadinhos/providers";
...
export async function POST(request: Request) {
  ...
  try {
    const importers = process.env.DEMO_MODE === "true" ? [new MockProductImporter()] : undefined;
    return NextResponse.json(await new ProductImportService(importers).import(parsed.data.url));
  }
  catch (error) {
