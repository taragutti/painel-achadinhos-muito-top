...
O projeto foi publicado como `v0.1.0-rc.3` e o fluxo de imagens públicas da Shopee foi integrado pelo PR #3. A CI passou no commit de merge `e0820f0a65266b211f682a41325a047016765b50`; o deploy web `dpl_BPiu9bF6uteu7F7SoBWx3tqq1zsH` está `READY` na Vercel e atende o domínio canônico. A entrega real continua desabilitada. O fluxo aprovado usa diretamente a imagem pública da Shopee durante a janela curta de retenção e bloqueia uploads manuais na produção, eliminando a exigência de Blob para esse caso de uso. O aceite autenticado do banco, o worker permanente e a tag final `v0.1.0` permanecem pendentes; consulte `docs/RELEASE_HANDOFF.md`. Se o empacotamento legado `vinext` voltar a ser usado, ele deve ser migrado e auditado separadamente antes da execução.
